...
// register Fluid_Magazine_Recent_Post widget
function fluid_magazine_register_recent_post_widget() {
    register_widget( 'Fluid_Magazine_Recent_Post' );
}
add_action( 'widgets_init', 'fluid_magazine_register_recent_post_widget' );


if ( ! class_exists( 'Fluid_Magazine_Recent_Post' ) ): 
 /**
 * Adds Fluid_Magazine_Recent_Post widget.
 */
class Fluid_Magazine_Recent_Post extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            'fluid_magazine_recent_posts', // Base ID
            __( 'RARA: Recent Post', 'fluid-magazine' ), // Name
            array( 'description' => __( 'A Recent Post Widget', 'fluid-magazine' ), ) // Args
        );
    }
                    ...
                    <li>                        
		    	        <?php if( has_post_thumbnail() && $show_thumb ){ ?>
                            <a href="<?php the_permalink();?>" class="post-thumbnail">
                                <?php the_post_thumbnail( 'fluid-magazine-recent-post' ); ?>
                            </a>
                        <?php }elseif( ! has_post_thumbnail() && $show_thumb ){ ?>
                            <a href="<?php the_permalink();?>" class="post-thumbnail">
                                <?php fluid_magazine_get_fallback_svg( 'fluid-magazine-recent-post' ); ?>
                            </a>
                        <?php } ?>
                                ...
                                <div class="entry-meta">
                                    <span class="posted-on">
			                            <a href="<?php the_permalink(); ?>">
                                            <time datetime="<?php  echo esc_attr( get_the_date( get_option( 'date_format' ) ) ); ?>">
                                            <?php echo esc_html( get_the_date( get_option( 'date_format' ) ) ); ?></time>
					                    </a>
                                    </span>
                                </div>
                            <?php } ?>
    ...
    public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'num_post' ) ); ?>"><?php esc_html_e( 'Number of Posts', 'fluid-magazine' ); ?></label> 
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_thumbnail' ) ); ?>"><?php esc_html_e( 'Show Post Thumbnail', 'fluid-magazine' ); ?></label>
        </p>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_postdate' ) ); ?>"><?php esc_html_e( 'Show Post Date', 'fluid-magazine' ); ?></label>
        </p>
        <?php 
    }
...
} // class Fluid_Magazine_Recent_Post 
endif;